...
symbolData.forEach(({ pos, clue }) => {
  ...
  const knot = new THREE.Mesh(new THREE.TorusKnotGeometry(0.25, 0.05, 56, 8), new THREE.MeshStandardMaterial({ color: 0x5b0f18, emissive: 0x1a0305, roughness: 0.6 }));
...
hum.connect(humGain);
hum.setRefDistance(15);
hum.position.set(0, 2, 0);
scene.add(hum);
oscillator.start();
...
window.addEventListener("keydown", (e) => {
  ...
  if (e.code === "KeyQ") showMessage("You are alone. Mostly.");
